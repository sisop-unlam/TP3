...
int main(int argc, char *argv[])
{
   ...
   if(pideAyuda(argc, argv)==PIDE_AYUDA) { mostrar_ayuda(); return PIDE_AYUDA; }
   ...
   strcpy(fifo_entrada,argv[1]);
    ...
    while(bytesLeidos>0){
        ...
        close (fd_entrada);
        fd_entrada= open(fifo_entrada,O_RDWR);
        memset(linea_entrada,0,TAM_CAD); linea_entrada[0]='\0';
...
if(strcmp(entrada->key,"ARTICULO")==0) orden = 1;
if(strcmp(entrada->key,"PRODUCTO")==0) orden = 2;
...
void devolverInfo(t_salida registros[], int cantHits, int fd_salida){
        ...
        for(i=0; i<cantHits; i++){
                memset(linea_salida1,0,230); linea_salida1[0]='\0';
...
int pideAyuda(int argc, char *argv[]){
    if (argc==2 && strcmp(argv[1],"--help")==0) {return PIDE_AYUDA;}
    return CONTINUAR;
}
